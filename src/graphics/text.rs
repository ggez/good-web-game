...
impl Font {
    ...
    pub fn new_glyph_font_bytes(context: &mut Context, bytes: &[u8]) -> GameResult<Self> {
        ...
        let v = bytes.to_vec();
        let font_id = context.internal.gfx_context.glyph_brush.add_font_bytes(v);
        Ok(Font { font_id })
    }
...
where
    ...
    G: GlyphPositioner,
{
    let brush = &mut context.internal.gfx_context.glyph_brush;
    match custom_layout {
    ...
    D: Into<DrawParam>,
{
    ...
    let gb = &mut ctx.internal.gfx_context.glyph_brush;
    let gc = &ctx.internal.gfx_context.glyph_cache.texture;
    ...
    match action {
        Ok(glyph_brush::BrushAction::ReDraw) => {
            let spritebatch = ctx.internal.gfx_context.glyph_state.clone();
        ...
        Err(glyph_brush::BrushError::TextureTooSmall { suggested }) => {
            ...
            ctx.internal.gfx_context.glyph_cache = new_glyph_cache.clone();
            let spritebatch = ctx.internal.gfx_context.glyph_state.clone();
            let spritebatch = &mut *spritebatch.borrow_mut();
...
fn update_texture(
    ...
    let offset = [rect.min.x as _, rect.min.y as _];
    let size = [rect.width() as _, rect.height() as _];
